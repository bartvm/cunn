...
template <typename T>
__global__ void countArcs_c(int batchSize, int totalInputs,
...
__global__ void reverseArcs_c(int batchSize, int totalInputs, int seqLength,
...
template <typename T>
__forceinline__ __device__ T sigmoid_c(T in) {
  T one = ScalarConvert<float, T>::to(1.0);
  return one / (one + THCNumerics<T>::exp(-in));
}

template <typename T>
__global__ void lstmElemwise_c(int t, int hiddenSize, int batchSize,
  ...
  T out_gate = g[0];
  T forget_gate = sigmoid_c<T>(g[1]);
  T in_gate = sigmoid_c<T>(g[2]);
...
template <typename T>
__global__ void calculateState_c(int hiddenSize, int batchSize,
                                 T* cellOutput, T* outputGates,
  ...
  output[index] = THCNumerics<T>::tanh(cellOutput[index]) * sigmoid_c<T>(outputGates[index]);
}

template <typename T>
__global__ void calculateGradState_c(int hiddenSize, int batchSize,
  ...
  T outputGateActivation = sigmoid_c<T>(outputGates[index]);
...
template <typename T>
__global__ void gradLstmElemwise_c(int t, int hiddenSize, int batchSize,
                                   int* targets, int* batches, int numOutArcs_t,
...
#include "generic/ConditionedMultiscaleLSTM.cu"
